...
# Anthropic client used for the AI chat module.
# We pass the API key explicitly so it works without env var lookups.
...
# is genuine, and save the user's info AND tokens to the session.
@app.route("/callback")
def callback():
    ...
    # Level 3: keep the full token response so /api-demo can use the access_token.
...
def stepup_callback():
    ...
    # Update the session with fresh claims AND fresh tokens.
    session["claims"]    = claims
    session["tokens"]    = tokens
...
# ─────────────────────────────────────────────────────────────
# ROUTES — Module 4: token-gated API (Level 3)
# ─────────────────────────────────────────────────────────────

# Shows the API demo page. Passes the raw access token to the
# template so the developer can see it and make live API calls.
@app.route("/api-demo")
@login_required
def api_demo():
    return render_template(
        ...
        token_exp=session["claims"].get("exp"),
    )


# A protected JSON endpoint. Calls Auth0's /userinfo endpoint
# using the access token from the session. This shows how a real
# API call works — the token travels as a Bearer header.
# Returns the userinfo JSON directly to the browser.
...
# by calling Auth0's token introspection endpoint.
# Returns active=true if valid, active=false if expired or invalid.
# This is how a backend service verifies a token it receives.
@app.route("/api/introspect", methods=["POST"])
@login_required
def api_introspect():
    access_token = session["tokens"]["access_token"]
    response = http.post(
        f"https://{AUTH0_DOMAIN}/oauth/token/introspect",
        data={
            "token":           access_token,
            "client_id":       AUTH0_CLIENT_ID,
            "client_secret":   AUTH0_CLIENT_SECRET,
...
# ─────────────────────────────────────────────────────────────
# ROUTES — Module 5: identity-aware AI (Level 3)
# ─────────────────────────────────────────────────────────────

# Shows the AI chat page.
# Requires MFA so we know the user's identity is verified
# before injecting it into the AI prompt.
@app.route("/ai")
@login_required
@mfa_required
def ai_page():
...
# Receives a chat message and returns an AI response.
# The key idea: we read the user's verified identity from the
# Auth0 session and inject it into Claude's system prompt.
# Claude then knows exactly who it is talking to and why
# they are verified — without the user having to say anything.
...
def ai_chat():
    ...
    role  = "admin" if is_admin() else "developer"
    amr   = session["claims"].get("amr", [])
    sub   = session["user"]["sub"]

    # Build a system prompt that tells Claude exactly who this user is.
    # All of this information came from the verified Auth0 ID token —
    # it was not typed by the user, so it can be trusted.
    system_prompt = f"""You are a helpful assistant inside OktaPath AI,
a developer education app about authentication with Auth0.
The user you are talking to has been verified by Auth0:
  Name:             {name}
  Email:            {email}
  Role:             {role}
  Auth methods:     {", ".join(amr)}
  User ID:          {sub}
Always address them by their first name.
If they are an admin, you can discuss advanced Auth0 configuration.
If they are a developer, keep explanations beginner-friendly.
Keep answers concise and practical. You can use short code examples.
If they ask something unrelated to auth or coding, answer helpfully
but gently steer back to the topic of authentication."""

    # Wrap the Anthropic call so any error (bad key, bad model, network)
    # comes back to the browser as JSON instead of an HTML crash page.
    try:
        ...
        print(f"[OktaPath] Anthropic call failed: {type(e).__name__}: {e}")
        return jsonify({"error": f"AI call failed — {type(e).__name__}: {e}"}), 500
